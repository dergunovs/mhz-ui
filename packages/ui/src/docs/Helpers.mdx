...
`api: typeof axios` единый инстанс axios
...
`handleError(error: unknown): string` возврат ошибки из error.response.data.message или "Ошибка" по умолчанию
...
`formatDuration(duration?: number, lang?: TLocale): string` преобразование времени в секундах в формат '1 мин. 09 сек.'

`formatDate(dateRaw?: string | Date | null, lang?: TLocale): string` преобразование даты в формат '7 февр. 2025 г.' с учётом локали или '0' при ошибке
`formatDateTime(dateRaw?: string | Date | null, lang?: TLocale): string` преобразование даты с временем в формат '7 февр. 2025 г. 14:41' с учётом локали или '0' при ошибке
`subtractDates(dateFuture?: string | Date | null, datePast?: string | Date | null, lang?: TLocale, isRawResult?: boolean): string | number` подсчёт промежутка между датами с преобразованием в формат '1 мин. 09 сек.' или секунды (isRawResult) или '0' при ошибке
...
`getFirstAndLastDays(count: number, isMonth: boolean): IWeekDays[]` получение первых и последних дней недель с датами и заголовками, выбор количества недель через count, возвращает массив объектов с полями dateFrom, dateTo и label
...
`deleteTempId<T extends { _id?: string }>(array: T[], isDeleteAll?: boolean): T[]` удаление у объектов внутри массива поля \_id, если внутри него хранится временный id с префиксом temp или всех полей \_id(isDeleteAll), возвращает пустой массив при ошибке
...
`localeField<T>(name: string, locale: TLocale)` возвращает поле с локалью (например, name или name_en)
...
`html(strings: TemplateStringsArray, ...expressions: unknown[])` хелпер для форматирования строк внутри шаблонов Storybook

## test

`dataTest(value: string): string` возврат элемента с указанным data-test аттрибутом со скобками и кавычками

`removeDataTest(node: RootNode | TemplateChildNode)` убрать аттрибуты data-test при билде в продакшен

`wait(time?: number): Promise<void>` подождать указанное количество милисекунд внутри теста для проверки setTimeout

`withSetup<T>(composable: () => Promise<T>)` функция для настройки тестов с использованием Vue композаблов (асинхронная)
