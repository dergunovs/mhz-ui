...
`subtractDates(dateRaw1?: string | Date | null, dateRaw2?: string | Date | null, isRawResult?: boolean): string | number` подсчёт промежутка между датами с преобразованием в формат '1 мин. 09 сек.' или секунды (isRawResult)
...
`formatPercent(percent: number): string` преобразование процентов в формат '+1%'

`getPercentDiff(cur: number, prev: number): number` получение разницы между двумя значениями в процентах

## password

`generatePassword(): string` генерация случайного 8 символьного пароля из букв и цифр
