...
`setCookieToken(token: string, tokenName: string)` - установка cookie с токеном авторизации на год
...
Композабл для авторизации пользователя. Предоставляет функции авторизации.

## useCalendar

```
useCalendar(): {
    dateFrom: Ref<Date | undefined>;
    dateTo: Ref<Date | undefined>;
    isDatesReady: Ref<boolean>;
    updateDates: (dates: ICalendarDates) => void;
}
```

Композабл для календаря, предоставляющий первую и последнюю дату, готовность календаря и функцию обновления дат.
...
usePwa(): {
    ...
    isPWACanBeInstalled: Ref<boolean>;
}
```
...
```
useValidate<T>(formData: Ref<T>, rules: Partial<{ [fieldName in keyof T]: (RuleItem | ((locale: TLocale) => RuleItem))[] }>, locale?: TLocale): {
...
- `required(locale?: TLocale): RuleItem`
- `email(locale?: TLocale): RuleItem`
- `letters(locale?: TLocale): RuleItem`
- `min(value: number, locale?: TLocale): RuleItem`
- `max(value: number, locale?: TLocale): RuleItem`

## useRouteId

```
useRouteId(title: string, isQuery?: boolean): {
  id: ComputedRef<string>;
}
```

Получение ID из маршрута (параметр или query).
...
Композабл для работы с темной темой. Возвращает реактивное состояние `isDarkMode` и функцию переключения темы `toggleDarkMode`. Тема сохраняется в localStorage.
...
Композабл, позволяющий запретить блокировку экрана на мобильном устройстве на конкретной странице приложения. Автоматически устанавливает и снимает WakeLock при монтировании/размонтировании компонента.
